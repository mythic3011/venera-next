...
Your comic source is registered in Venera's unified source platform resolver when first loaded. The resolver handles:

1. **Canonical key registration** — Your `key` is stored in the `source_platforms` table
2. **Legacy key mapping** — If your source replaces an old source, the team can add legacy aliases
3. **Context resolution** — The resolver ensures your source is identified consistently across user favorites, history, reader state, and imports

**For source developers:** If you rename or refactor your source, coordinate with the Venera team to avoid breaking existing user libraries.

**Reference:** [lib/foundation/sources/source_ref.dart](../../lib/foundation/sources/source_ref.dart)
...
class NewComicSource extends ComicSource {
  ...
  // This key is registered in the platform resolver and must be globally unique.
  // Changing this key breaks existing user libraries—only change if absolutely necessary.
  key = "";
...
The `key` field is the canonical identifier for your source. It's used by Venera's unified source platform resolver to:

- Track source identity across app contexts (favorites, history, reader, imports)
- Handle legacy source migrations
- Enable cross-platform source resolution

**Important:** Once users adopt a source, avoid changing the `key`. If you must deprecate a key, coordinate with the Venera team to add a legacy mapping in `source_platform_aliases`.
